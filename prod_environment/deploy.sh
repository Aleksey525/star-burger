...
echo "Сборка и запуск контейнеров..." docker-compose build
docker-compose up -d

echo "Запуск Certbot для получения SSL сертификата..."
docker-compose exec certbot certonly --webroot --webroot-path=/var/www/certbot/ --email alex_tolchin@mail.ru --agree-tos --no-eff-email -d star-burg.ru -d www.star-burg.ru
